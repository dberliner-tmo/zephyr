...
static enum net_verdict handle_native_ipv4_echo_reply(struct net_pkt *pkt,
					       struct net_ipv4_hdr *ip_hdr,
					       struct net_icmp_hdr *icmp_hdr);
    
static enum net_verdict handle_native_ipv6_echo_reply(struct net_pkt *pkt,
...
static enum net_verdict handle_native_ipv6_echo_reply(struct net_pkt *pkt, struct net_ipv6_hdr *ip_hdr,
						        struct net_icmp_hdr *icmp_hdr)
{
    NET_PKT_DATA_ACCESS_CONTIGUOUS_DEFINE(icmp_access,
...
int net_ping(struct net_if *iface, struct sockaddr *dst, size_t size)
{
    ...
    if (!net_if_is_ip_offloaded(iface)){
        ...
        if (dst->sa_family == AF_INET) { 
            ...
            stat = net_icmpv4_send_echo_request(iface, (struct in_addr*)dst, sys_rand32_get(), 1, data, size);
            return stat;
        } else if (dst->sa_family == AF_INET6) {
            net_icmpv6_register_handler(&ping6_handler);
            stat =  net_icmpv6_send_echo_request(iface, (struct in6_addr*)dst, sys_rand32_get(), 1, data, size);
            return stat;
        }
    } 
#endif
