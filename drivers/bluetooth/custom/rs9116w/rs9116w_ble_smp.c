...
typedef struct rsi_smp_event_s{
	uint8_t event_type;
	uint16_t status;
	union {
		uint8_t dev_addr[6];
 ...
 */
static rsi_smp_event_t *get_event_slot()
{
	int old_event_ptr = smp_event_ptr;
	smp_event_ptr++;
	smp_event_ptr %= CONFIG_RSI_BT_EVENT_QUEUE_SIZE;
	rsi_smp_event_t *target_event = &smp_event_queue[smp_event_ptr];
...
void rsi_ble_on_smp_response (rsi_bt_event_smp_resp_t  *smp_resp)
{
	...
	rsi_smp_event_t *target_evt = get_event_slot();
	if (!target_evt) {
		BT_ERR("Event queue full!");
		return;
	}
	target_evt->event_type = RSI_EVT_SMP_RESP;
	memcpy(&target_evt->smp_resp, smp_resp, sizeof(rsi_bt_event_smp_resp_t));
	rsi_bt_raise_evt();
}
...
void rsi_ble_on_smp_passkey_display (rsi_bt_event_smp_passkey_display_t *smp_passkey_display)
{
	...
	memcpy(target_evt->passkey_disp.dev_addr, smp_passkey_display->dev_addr, 6);
	target_evt->passkey_disp.passkey += (smp_passkey_display->passkey[0] - '0');
	target_evt->passkey_disp.passkey += (smp_passkey_display->passkey[1] - '0') * 10;
	target_evt->passkey_disp.passkey += (smp_passkey_display->passkey[2] - '0') * 100;
	target_evt->passkey_disp.passkey += (smp_passkey_display->passkey[3] - '0') * 1000;
	target_evt->passkey_disp.passkey += (smp_passkey_display->passkey[4] - '0') * 10000;
	target_evt->passkey_disp.passkey += (smp_passkey_display->passkey[5] - '0') * 100000;
	rsi_bt_raise_evt();
}
...
void rsi_ble_on_sc_passkey (rsi_bt_event_sc_passkey_t *sc_passkey)
{
	...
	memcpy(&target_evt->passkey_disp, sc_passkey, sizeof(rsi_bt_event_sc_passkey_t));
	rsi_bt_raise_evt();
}
...
void rsi_ble_on_encrypt_started(uint16_t status, rsi_bt_event_encryption_enabled_t *enc_enabled)
{
	...
	target_evt->status = status;
	memcpy(&target_evt->enc_start, enc_enabled, sizeof(rsi_bt_event_encryption_enabled_t));
	rsi_bt_raise_evt();
}
...
static void rsi_ble_on_le_ltk_req_event(rsi_bt_event_le_ltk_request_t *le_ltk_req)
{
	...
	target_evt->event_type = RSI_EVT_LTK_REQ;
  	memcpy(&target_evt->ltk_req, le_ltk_req, sizeof(rsi_bt_event_le_ltk_request_t));
	rsi_bt_raise_evt();
}
...
void rsi_ble_on_le_security_keys(rsi_bt_event_le_security_keys_t *rsi_ble_event_le_security_keys)
{
  ...
  rsi_smp_event_t *target_evt = get_event_slot();
	...
	target_evt->event_type = RSI_EVT_SEC_KEYS;
  	memcpy(&target_evt->le_sec, rsi_ble_event_le_security_keys, sizeof(rsi_bt_event_le_security_keys_t));
	rsi_bt_raise_evt();
}
...
void bt_smp_init()
{
	// sc_supported = !IS_ENABLED(CONFIG_BT_SMP_OOB_LEGACY_PAIR_ONLY);
...
static int smp_send_security_req(struct bt_conn *conn)
{
	...
	// k_msleep(1000);
	return rsi_ble_smp_pair_request(conn->le.dst.a.val, get_io_capa(), mitm);
	// k_msleep(1000);
}
...
int bt_smp_auth_passkey_confirm(struct bt_conn *conn) // TODO: Check if this is actually needed
{
	return -ENOTSUP;
}
...
void bt_smp_process()
{
	rsi_smp_event_t *current_event = &smp_event_queue[smp_event_ptr];
	while (current_event->event_type){
		...
		switch (current_event->event_type) {
			...
			case RSI_EVT_PASSKEY_DISP:
			{
				...
				bt_addr_le_t addr;
				memcpy(addr.a.val, current_event->passkey_disp.dev_addr, 6);

				selected_conn = bt_conn_lookup_addr_le(0, &addr);

				if (!selected_conn){
					BT_WARN("Passkey display event: Unable to find connection");
					break;
				}

				struct bt_smp *smp = &bt_smp_pool[selected_conn->handle];

				smp->passkey = current_event->passkey_disp.passkey;

				if (bt_auth->passkey_display){
					bt_auth->passkey_display(selected_conn, current_event->passkey_disp.passkey);
				} else {
					if (!current_event->passkey_disp.passkey){
						if (bt_auth->passkey_entry){
							bt_auth->passkey_entry(selected_conn);
						
						} else {
							BT_WARN("Missing callback to process passkey");
						}
					} else if (bt_auth->passkey_confirm) {
						bt_auth->passkey_confirm(selected_conn, current_event->passkey_disp.passkey);
					} else {
						BT_WARN("Missing callback to process passkey");
					}
				}
				...
				if (current_event->passkey_disp.passkey) {
					...
					rsi_ble_smp_passkey(current_event->passkey_disp.dev_addr, current_event->passkey_disp.passkey);
				}

				bt_conn_unref(selected_conn);
				break;
			}
			case RSI_EVT_SMP_FAIL: //One case is actually cancel
			{
				struct bt_conn *selected_conn;
				...
				if (!selected_conn){
					BT_WARN("SMP fail event: Unable to find connection");
					break;
				}
			...
			case RSI_EVT_ENC_START:
			{
				...
				bt_addr_le_t addr;
				memcpy(addr.a.val, current_event->enc_start.dev_addr, 6);

				selected_conn = bt_conn_lookup_addr_le(0, &addr);

				if (!selected_conn){
					BT_WARN("Encrypt start event: Unable to find connection");
					break;
				}
				...
				if (!current_event->status){ 
					if (!selected_conn->le.keys){
						selected_conn->le.keys = bt_keys_get_addr(0, &selected_conn->le.dst);
					} //Check if still null
					if (!selected_conn->le.keys){
						/* Not sure if this is the proper solution, however, it should suffice */
						security_changed(selected_conn, BT_SECURITY_ERR_UNSPECIFIED);
						break;
					}
					bool newkey = true;
					uint8_t flags = 0;
					if (!memcmp(current_event->enc_start.localltk, selected_conn->le.keys->ltk.val, 16)) {
						flags = selected_conn->le.keys->flags;
						newkey = false;
					} else {
						if (current_event->enc_start.enabled & BIT(3)) {
							flags = BT_KEYS_AUTHENTICATED | BT_KEYS_SC;
						} else if (current_event->enc_start.enabled & BIT(1)) {
							flags = BT_KEYS_AUTHENTICATED;
						} 
					}
					bt_security_t old_sec_level = selected_conn->sec_level;
					if (flags & (BT_KEYS_AUTHENTICATED | BT_KEYS_SC)){ //Todo: ENC KEY SIZE
						...
						BT_WARN("Unknown encrypted security level; Defaulting to L2");
						selected_conn->sec_level = BT_SECURITY_L2;	
					} else {
					...
					if (newkey){
						memcpy(selected_conn->le.keys->ltk.val, current_event->enc_start.localltk, 16);
						memcpy(selected_conn->le.keys->ltk.rand, current_event->enc_start.localrand, 8);
						// memcpy(selected_conn->le.keys->ltk.ediv, current_event->enc_start.localediv, 2); //?
			...
			case RSI_EVT_LTK_REQ: 	
			{
				...
				if (!selected_conn){
					BT_WARN("LTK Request: Unable to find connection");
					break;
				}

				/* First check for keys: */
				if (!selected_conn->le.keys){
					selected_conn->le.keys = bt_keys_find_addr(0, &selected_conn->le.dst);
				}
				/* Verify keys */
				if (selected_conn->le.keys 
					&& (rsi_bytes2R_to_uint16(selected_conn->le.keys->ltk.ediv) == current_event->ltk_req.localediv)
            		&& !(memcmp(selected_conn->le.keys->ltk.rand, current_event->ltk_req.localrand, 8))) {
          			rsi_ble_ltk_req_reply(selected_conn->le.dst.a.val, 1, selected_conn->le.keys->ltk.val);
				} else {
					rsi_ble_ltk_req_reply(selected_conn->le.dst.a.val, 0, NULL);
				}
				bt_conn_unref(selected_conn);
				break;
			}
			case RSI_EVT_SMP_RESP: {
				...
				if (!selected_conn){
					BT_WARN("SMP response: Unable to find connection");
					break;
				}

				if ((get_io_capa() != BT_SMP_IO_NO_INPUT_OUTPUT) && 
					(IS_ENABLED(CONFIG_BT_SMP_ENFORCE_MITM) ||  selected_conn->required_sec_level >= 3
					))
				{
					mitm = 1;
				}
				rsi_ble_smp_pair_response(current_event->smp_resp.dev_addr, get_io_capa(), mitm);
			...
			case RSI_EVT_SEC_KEYS:
			{
				...
				if (!selected_conn){
					BT_WARN("SMP security keys: Unable to find connection");
					break;
				}

				identity_addr.type = current_event->le_sec.Identity_addr_type;
				memcpy(identity_addr.a.val, current_event->le_sec.Identity_addr, 6);
		...
		smp_event_ptr--;
		smp_event_ptr = smp_event_ptr < 0 ? ARRAY_SIZE(smp_event_queue): smp_event_ptr;
