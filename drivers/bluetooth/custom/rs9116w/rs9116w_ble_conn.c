...
struct bt_conn *bt_conn_ref(struct bt_conn *conn)
{
	...
	BT_DBG("handle %u ref %u -> %u", conn->handle, (uint32_t)old, (uint32_t)(old + 1));
				     ...
				     const struct bt_le_conn_param *param)
{
	...
	rc = rsi_ble_conn_params_update(conn->le.dst.a.val, param->interval_min, param->interval_max, param->latency, param->timeout);
...
int bt_conn_set_security(struct bt_conn *conn, bt_security_t sec) //Should be fine
{
	int err;
...
int bt_conn_auth_cb_register(const struct bt_conn_auth_cb *cb) //Should be fine
{
	if (!cb) {
		bt_auth = NULL;
		return 0;
...
int bt_conn_auth_passkey_entry(struct bt_conn *conn, unsigned int passkey) //Todo? (90% sure this is client only)
{
	// return -ENOTSUP;
	if (!bt_auth) {
		return -EINVAL;
...
int bt_conn_auth_cancel(struct bt_conn *conn) //Todo
{
	//Not sure if there is actually a way to do this...
	return -ENOTSUP;
	if (!bt_auth) {
		return -EINVAL;
	}
