...
typedef struct rsi_attr_handle_table_entry_s {
	uint16_t handle;
	const struct bt_gatt_attr *attr;
} rsi_attr_handle_table_entry_t;

rsi_attr_handle_table_entry_t att_handle_table[RSI_BLE_MAX_NBR_ATT_REC] = { 0 };
...
rsi_gatt_event_t gatt_event_queue[CONFIG_RSI_BT_EVENT_QUEUE_SIZE] = { 0 };
// rsi_gatt_event_t *current_event = gatt_event_queue;
int gatt_event_ptr = 0;
 ...
 */
static rsi_gatt_event_t *get_event_slot()
{
	int old_event_ptr = gatt_event_ptr;
	gatt_event_ptr++;
	gatt_event_ptr %= CONFIG_RSI_BT_EVENT_QUEUE_SIZE;
	rsi_gatt_event_t *target_event = &gatt_event_queue[gatt_event_ptr];
...
#if CONFIG_BT_ATT_PREPARE_COUNT > 0
rsi_ble_event_prepare_write_t prepared_writes[CONFIG_BT_ATT_PREPARE_COUNT] = { 0 };
size_t prepared_writes_count = 0;
#else
...
{
	rsi_gatt_event_t *target_event = get_event_slot();
	...
	if (prepared_writes_count < CONFIG_BT_ATT_PREPARE_COUNT) {
		memcpy(&prepared_writes[prepared_writes_count], rsi_app_ble_prepared_write_event, sizeof(rsi_ble_event_prepare_write_t));
	...
	#else
	last_pw_handle = rsi_bytes2R_to_uint16(rsi_app_ble_prepared_write_event->handle);
	memcpy(target_event->pw.addr.val, rsi_app_ble_prepared_write_event->dev_addr, 6);
	target_event->pw.handle = rsi_bytes2R_to_uint16(rsi_app_ble_prepared_write_event->handle);
	#endif
	rsi_bt_raise_evt();
}
					   ...
					   rsi_ble_execute_write_t *rsi_app_ble_execute_write_event)
{
	...
	memcpy(&target_event->ew, rsi_app_ble_execute_write_event, sizeof(rsi_ble_execute_write_t));
	rsi_bt_raise_evt();
}
					    ...
					    struct _bt_gatt_ccc *ccc)
{
	for (size_t i = 0; i < ARRAY_SIZE(ccc->cfg); i++) {
		...
		} else if (!memcmp(&cfg->peer.a.val, BT_ADDR_LE_ANY->a.val, 6)) {
			return cfg;
		}
	}
 ...
 */
static uint16_t rsi_attr_handle_table_insert(uint16_t handle, const struct bt_gatt_attr *attr)
{
	uint16_t target;
...
{
	return (perms & BT_GATT_PERM_READ) ? RSI_BLE_ATT_PROPERTY_READ : 0
	       | (perms & BT_GATT_PERM_WRITE) ? RSI_BLE_ATT_PROPERTY_WRITE : 0
	       | (perms & BT_GATT_PERM_READ_ENCRYPT) ? RSI_BLE_ATT_PROPERTY_READ : 0
	       | (perms & BT_GATT_PERM_WRITE_ENCRYPT) ? RSI_BLE_ATT_PROPERTY_WRITE : 0
	       | (perms & BT_GATT_PERM_READ_AUTHEN) ? RSI_BLE_ATT_PROPERTY_READ : 0
	       | (perms & BT_GATT_PERM_WRITE_AUTHEN) ? RSI_BLE_ATT_PROPERTY_WRITE : 0;
}
 ...
 */
static void rsi_ble_add_ccc_att(void *serv_handler,
				uint16_t handle)
{
...
static void bt_gatt_service_init(void)
{
	STRUCT_SECTION_FOREACH(bt_gatt_service_static, svc) {
		...
		for (int i = 0; i < svc->attr_count; i++) {
			...
			bool use_security = (attr->perm & (BT_GATT_PERM_READ_ENCRYPT | BT_GATT_PERM_WRITE_ENCRYPT | BT_GATT_PERM_READ_AUTHEN | BT_GATT_PERM_WRITE_AUTHEN));
			...
			if (!bt_uuid_cmp(attr->uuid, BT_UUID_GATT_CHRC)) {
				...
				for (int i = 0; i < 3; i++) {
					if (prop_known_uuids[i] == NULL) {
						prop_known_uuids[i] = chrc->uuid;
						prop_knowns[i] = chrc->properties;
						break;
					}
				}
				if (rsi_attr_handle_table_insert(handle, attr) == UINT16_MAX) {
					BT_ERR("Failed to add attribute, table full!");
					return;
				}
				...
				uint8_t props = 0;
				for (int i = 0; i < 3; i++) {
					if (prop_known_uuids[i] && !bt_uuid_cmp(prop_known_uuids[i], attr->uuid)) {
						props = prop_knowns[i];
						prop_known_uuids[i] = NULL;
						break;
					}
				}
				if (!props) {
...
void bt_gatt_disconnected(struct bt_conn *conn)
{
	...
	for (int i = 0; i < ARRAY_SIZE(prepared_writes); i++) {
		if (!memcmp(prepared_writes[i].dev_addr, conn->le.dst.a.val, 6)) {
			for (int j = i + 1; i < ARRAY_SIZE(prepared_writes); j++) {
				memcpy(&prepared_writes[j - 1], &prepared_writes[j], sizeof(rsi_ble_event_prepare_write_t));
			}
			memset(&prepared_writes[ARRAY_SIZE(prepared_writes) - 1], 0, sizeof(rsi_ble_event_prepare_write_t));
		}
	}
	#endif
	const struct bt_gatt_attr *attr;
...
void bt_gatt_init(void)
{
	...
	gatt_event_queue[ARRAY_SIZE(gatt_event_queue) - 1].next = gatt_event_queue;
}
			       ...
			       bt_gatt_attr_func_t func, void *user_data)
{
	...
	if (start_handle <= last_static_handle) {
		...
		STRUCT_SECTION_FOREACH(bt_gatt_service_static, static_svc) {
			...
			if (bt_gatt_attr_get_handle(&static_svc->attrs[static_svc->attr_count - 1]) < start_handle) {
				continue;
			} else {
				handle = bt_gatt_attr_get_handle(static_svc->attrs);
			}
...
uint16_t bt_gatt_attr_get_handle(const struct bt_gatt_attr *attr)
{
	...
	for (uint16_t i = 0; i <= RSI_BLE_MAX_NBR_ATT_REC; i++) {
		if (att_handle_table[i].attr != NULL && att_handle_table[i].attr == attr) {
...
{
	rsi_gatt_event_t *current_event = &gatt_event_queue[gatt_event_ptr];
	...
	while (current_event->event_type) {
		...
		case RSI_EVT_READ:
		{
			curr_att = rsi_attr_handle_table_search(current_event->r.handle);
			if (curr_att == NULL) {
				rsi_ble_att_error_response(current_event->ew.dev_addr, current_event->r.handle, 0x0a, BT_ATT_ERR_ATTRIBUTE_NOT_FOUND);
				break;
			}
			if (!(curr_att->perm & BT_GATT_PERM_READ)) {
				rsi_ble_att_error_response(current_event->r.dev_addr, current_event->r.handle, (current_event->r.type ? 0x0c : 0x0a), BT_ATT_ERR_READ_NOT_PERMITTED);
				break;
			}
			...
			if (!selected_conn){
				BT_WARN("GATT read request: Unable to find connection");
				break;
			}

			ssize_t read_size;
			if (curr_att->read != NULL) {
				read_size = curr_att->read(selected_conn, curr_att, buf, sizeof(buf), current_event->r.offset);
			...
			if (read_size < 0) {
				...
				if (!current_event->r.type) {
					status = rsi_ble_att_error_response(current_event->r.dev_addr, current_event->r.handle, 0x0a, err);
				} else {
					status = rsi_ble_att_error_response(current_event->r.dev_addr, current_event->r.handle, 0x0c, err);
				}
			} else {
				uint16_t read_len = read_size;
				status = rsi_ble_gatt_read_response(current_event->r.dev_addr,
								    current_event->r.type,
								    current_event->r.handle,
								    current_event->r.offset,
								    read_len,
								    buf);
			}
		...
		case RSI_EVT_WRITE:
		{
			...
			ssize_t return_val;
			uint16_t handle = rsi_bytes2R_to_uint16(current_event->w.handle);
			curr_att = rsi_attr_handle_table_search(handle);
			const struct bt_gatt_attr *chrc_desc_att = rsi_attr_handle_table_search(handle - 1);
			if (chrc_desc_att && !bt_uuid_cmp(chrc_desc_att->uuid, BT_UUID_GATT_CHRC)) {
				struct bt_gatt_chrc *chrc = chrc_desc_att->user_data;
				response_required = !(chrc->properties & BT_GATT_CHRC_WRITE_WITHOUT_RESP);
			}
			if (curr_att == NULL) {
				rsi_ble_att_error_response(current_event->w.dev_addr, handle, 0x12, BT_ATT_ERR_ATTRIBUTE_NOT_FOUND);
				break;
			}
			if (!(curr_att->perm & BT_GATT_PERM_WRITE)) {
				rsi_ble_att_error_response(current_event->w.dev_addr, handle, 0x12, BT_ATT_ERR_WRITE_NOT_PERMITTED);
				break;
			}
			struct bt_conn *selected_conn;
			...
			if (!selected_conn){
				BT_WARN("GATT write request: Unable to find connection");
				break;
			}

			if (curr_att->write != NULL) {
				return_val = curr_att->write(selected_conn, curr_att, current_event->w.att_value, current_event->w.length, 0, response_required ? 0 : BT_GATT_WRITE_FLAG_CMD);
				if (return_val < 0) {
					BT_ERR("Write Error %d\n", return_val);
					rsi_ble_att_error_response(current_event->w.dev_addr, handle, 0x12, -return_val);
				} else {
					if (response_required) {
						rsi_ble_gatt_write_response(current_event->w.dev_addr, 0);
					}
				}
			} else {
				if (response_required) {
		...
		case RSI_EVT_MTU:
		{
			rsi_ble_mtu_exchange_event(current_event->addr.val, BT_ATT_MTU);
			break;
		}
		case RSI_EVT_PREP_WRITE:
		{
			#if CONFIG_BT_ATT_PREPARE_COUNT > 0
			ssize_t return_val;
			uint16_t handle = rsi_bytes2R_to_uint16(current_event->pw->handle);
			if (current_event->pw == NULL) {
				rsi_ble_att_error_response(current_event->pw->dev_addr, handle, 0x16, BT_ATT_ERR_PREPARE_QUEUE_FULL);
				break;
			}
			curr_att = rsi_attr_handle_table_search(handle);
			if (curr_att == NULL) {
				rsi_ble_att_error_response(current_event->pw->dev_addr, handle, 0x16, BT_ATT_ERR_ATTRIBUTE_NOT_FOUND);
				prepared_writes_count--;
				break;
			}
			if (!(curr_att->perm & BT_GATT_PERM_WRITE)) {
				rsi_ble_att_error_response(current_event->pw->dev_addr, handle, 0x16, BT_ATT_ERR_WRITE_NOT_PERMITTED);
				prepared_writes_count--;
				break;
			} else if (!(curr_att->perm & BT_GATT_PERM_PREPARE_WRITE)) {
				rsi_ble_gatt_prepare_write_response(current_event->pw->dev_addr, handle, rsi_bytes2R_to_uint16(current_event->pw->offset), current_event->pw->length, current_event->pw->att_value);
				break;
			}
			struct bt_conn *selected_conn;
			...
			if (!selected_conn){
				BT_WARN("GATT prepare write request: Unable to find connection");
				break;
			}

			if (curr_att->write != NULL) {
				return_val = curr_att->write(selected_conn, curr_att, current_event->pw->att_value, current_event->pw->length, rsi_bytes2R_to_uint16(current_event->pw->offset), BT_GATT_WRITE_FLAG_PREPARE);
				if (return_val < 0) {
					BT_ERR("Prepare Write Error %ld\n", return_val);
					rsi_ble_att_error_response(current_event->pw->dev_addr, handle, 0x16, -return_val);
					prepared_writes_count--;
				} else {
					rsi_ble_gatt_prepare_write_response(current_event->pw->dev_addr, handle, rsi_bytes2R_to_uint16(current_event->pw->offset), current_event->pw->length, current_event->pw->att_value);
				}
			...
			bt_conn_unref(selected_conn);
			#else
			rsi_ble_att_error_response(current_event->pw.addr.val, current_event->pw.handle, 0x16, BT_ATT_ERR_NOT_SUPPORTED);
			#endif
		}
		case RSI_EVT_EXEC_WRITE:
		{
			...
			ssize_t return_val;
			int count = 0;
			rsi_ble_req_prepare_write_t *pw_conn[CONFIG_BT_ATT_PREPARE_COUNT] = { 0 };
			rsi_ble_req_prepare_write_t *pw_curr = NULL;

			for (int i = prepared_writes_count - 1; i >= 0; i--) {
				if (!memcmp(prepared_writes[i].dev_addr, current_event->ew.dev_addr, 6)) {
			...
			if (pw_curr == NULL) {
				rsi_ble_att_error_response(current_event->ew.dev_addr, 0, 0x18, BT_ATT_ERR_WRITE_NOT_PERMITTED);
				break;
			}
			struct bt_conn *selected_conn;
			while (count > 0) {
				uint16_t handle = rsi_bytes2R_to_uint16(pw_curr->handle);
				curr_att = rsi_attr_handle_table_search(handle);
				if (curr_att == NULL) {
					rsi_ble_att_error_response(current_event->ew.dev_addr, handle, 0x18, BT_ATT_ERR_ATTRIBUTE_NOT_FOUND);
					break;
				}
				if (!(curr_att->perm & BT_GATT_PERM_WRITE)) {
					rsi_ble_att_error_response(current_event->ew.dev_addr, handle, 0x18, BT_ATT_ERR_WRITE_NOT_PERMITTED);
					break;
				}


				bt_addr_le_t addr;
				memcpy(addr.a.val, current_event->ew.dev_addr, 6);

				selected_conn = bt_conn_lookup_addr_le(0, &addr);

				if (!selected_conn){
					BT_WARN("GATT execute write request: Unable to find connection");
					break;
				}

				if (curr_att->write != NULL) {
					return_val = curr_att->write(selected_conn, curr_att, pw_curr->att_value, pw_curr->length, rsi_bytes2R_to_uint16(pw_curr->offset), 0);
					if (return_val < 0) {
						BT_ERR("Execute Write Error %ld\n", return_val);
						rsi_ble_att_error_response(pw_curr->dev_addr, handle, 0x18, -return_val);
					}
				}
				count--;
				if (count > 0) {
					...
					rsi_ble_gatt_write_response(pw_curr->dev_addr, 1);
				}
			}
			...
			#else
			rsi_ble_att_error_response(current_event->ew.dev_addr, last_pw_handle, 0x18, BT_ATT_ERR_NOT_SUPPORTED);
			#endif
		}
		default:
			break;
...
{
	ARG_UNUSED(handle);
	return rsi_ble_notify_value(conn->le.dst.a.val, handle, params->len, (uint8_t*)params->data);
}
...
{
	return rsi_ble_indicate_value(conn->le.dst.a.val, handle, params->len, (uint8_t*)params->data);
}
