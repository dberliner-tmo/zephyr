...
static int ble_adv_connectable(const struct shell *shell, int argc, char **argv)
{
	...
	if (get_active_le_conns() < CONFIG_BT_MAX_CONN) {
		...
		if (err) {
			shell_error(shell, "Advertising failed to start (err %d)", err);
			return 1;
		}
		shell_print(shell, "Advertising successfully started");
		return 0;
	} else {
		shell_error(shell, "Maximum number of simultaneous connections reached");
		return 1;
	}
}
	...
	if (count) {
		...
		for (int i = 0; i < CONFIG_BT_MAX_CONN; i++) {
			if (get_acl_conn(i)->ref > 0
				...
				shell_print(shell, "\t%02d: ADDR = %02X-%02X-%02X-%02X-%02X-%02X", n,
...
SHELL_STATIC_SUBCMD_SET_CREATE(ble_9116_adv_sub,
...
SHELL_STATIC_SUBCMD_SET_CREATE(ble_9116_conn_sub,
...
SHELL_STATIC_SUBCMD_SET_CREATE(ble_9116_sub,
			       SHELL_CMD(adv, &ble_9116_adv_sub, "Advertise control command.", NULL),
			       SHELL_CMD(conn,  &ble_9116_conn_sub, "Connection control command.", NULL),
			       SHELL_SUBCMD_SET_END
			       );
