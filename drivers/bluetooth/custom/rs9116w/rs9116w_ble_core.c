...
void rsi_bt_rx_thread(void *, void *, void *);

#define DT_DRV_COMPAT redpinesignals_rs9116w
...
int bt_enable(bt_ready_cb_t cb)
{
	...
	/*spawn the thread*/
	k_thread_create(&rsi_bt_rx_thread_data, rsi_bt_rx_thread_stack, K_THREAD_STACK_SIZEOF(rsi_bt_rx_thread_stack), rsi_bt_rx_thread,
...
/**
 * @brief Simple interupt handler to signal thread
 */
void int_rx_event_cb(const struct device *port,
...
const char *bt_get_name(void)
{
	...
	if (rsi_bt_get_local_name(bt_resp_get_local_name)) {
		...
		memcpy(name_buf, bt_resp_get_local_name->name, bt_resp_get_local_name->name_len);
		return name_buf;
	}
#else
...
void rsi_bt_rx_thread(void *a, void *b, void *c)
{
	...
	while (1) {
#if !IS_ENABLED(CONFIG_WISECONNECT_USE_OS_BINDINGS)
		/* The 5 seconds here is for the edge case when an event is not fired when it should be */
