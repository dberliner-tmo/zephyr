			 ...
			 time_t *tp)
{
	...
	tm.tm_mday = bcd2bin(*dp++);
	tm.tm_mon  = bcd2bin(*dp++);
		     ...
		     const struct gecko_rtcc_alarm *cp)
{
	...
	if (rc >= 0) {
		...
		time |= (buf[0] << _RTCC_CC_TIME_SECU_SHIFT) &
				( _RTCC_CC_TIME_SECU_MASK |
				  _RTCC_CC_TIME_SECT_MASK );
		time |= (buf[1] << _RTCC_CC_TIME_MINU_SHIFT) &
				( _RTCC_CC_TIME_MINU_MASK |
				  _RTCC_CC_TIME_MINT_MASK );
		time |= (buf[2] << _RTCC_CC_TIME_HOURU_SHIFT) &
				( _RTCC_CC_TIME_HOURU_MASK |
				  _RTCC_CC_TIME_HOURT_MASK );
		date |= (buf[3] << _RTCC_CC_DATE_DAYU_SHIFT) &
				( _RTCC_CC_DATE_DAYU_MASK |
				  _RTCC_CC_DATE_DAYT_MASK );
		date |= (buf[4] << _RTCC_CC_DATE_MONTHU_SHIFT) &
				( _RTCC_CC_DATE_MONTHU_MASK |
				  _RTCC_CC_DATE_MONTHT_MASK );

		RTCC_ChannelDateSet(id, date);
		RTCC_ChannelTimeSet(id, time);
	}
	if ((rc >= 0)
	    && (cp->handler != NULL)) {
