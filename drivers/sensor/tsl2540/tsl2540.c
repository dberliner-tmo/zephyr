				...
				struct sensor_value *val)
{
	...
	k_sem_take(&data->sem, K_FOREVER);
	double cpl = (data->integration_time + 1) * 2.81;
	cpl *= data->again;
	uint32_t uval;
	switch (chan) {
...
int tsl2540_attr_set(const struct device *dev,
		      enum sensor_channel chan,
		      ...
		      const struct sensor_value *val)
{
	...
	if (chan == SENSOR_CHAN_IR || chan == SENSOR_CHAN_LIGHT) {
		...
		if (attr == SENSOR_ATTR_INTEGRATION_TIME) {
			...
			it -= 1;
			uint8_t itv = (uint8_t)it;
			if (tsl2540_reg_write(dev, TSL2540_REG_ATIME,
					   itv)) {
				ret = -EIO;
				goto exit;
			}
...
static int tsl2540_setup(const struct device *dev)
{
	...
	val.val1 = 400;
	tsl2540_attr_set(dev, SENSOR_CHAN_LIGHT, SENSOR_ATTR_INTEGRATION_TIME, &val);
			      ...
			      enum pm_device_action action)
{
	...
	case PM_DEVICE_ACTION_RESUME:
		ret = (dev, TSL2540_REG_EN, TSL2540_EN_ACTIVE);
		break;
...
#define TSL2540_DEFINE(inst)						   \
	...
	DEVICE_DT_INST_DEFINE(inst,					   \
			      ...
			      &tsl2540_prv_data_##inst,			   \
			      &tsl2540_config_##inst,			   \
			      POST_KERNEL,				   \
