/*
 * Copyright (c) 2021 Jared Baumann
 *
...
static int rs9116w_ioctl(void *obj, unsigned int request, va_list args)
{
	...
	default:
		errno = EINVAL;
		return -1;
		/* Similarly O_NONBLOCK not able to be set on runtime */
...
// static int rs9116w_off_getaddrinfo(const char *node, const char *service,
...
// 	if (hints && hints->ai_family != Z_AF_INET && hints->ai_family != Z_AF_INET6) {
// 		return DNS_EAI_FAIL;
// 	} else {
// 		ipver = hints->ai_family == Z_AF_INET ? 4 : 6;
// 	}

// 	/* DNS lookup */

// 	// snprintk(eswifi->buf, sizeof(eswifi->buf), "D0=%s\r", node);
// 	rsi_rsp_dns_query_t query_resp;
// 	err = rsi_dns_req(ipver, node, NULL, NULL, &query_resp, sizeof(query_resp));
// 	if (err < 0) {
// 		err = DNS_EAI_FAIL;
// 	}
...
// 	for (int i = 0; i < query_resp.ip_count; i++){
...
// 			}
// 		} else {
// 			ai_addr6 = calloc(1, sizeof(*ai_addr6));
// 			if (!ai_addr6) {
// 				free(*res);
// 				return DNS_EAI_MEMORY;
// 			}
// 		}
// 		ai->ai_family =  query_resp.ip_version == 4 ? Z_AF_INET : Z_AF_INET6;
...
// 			memcpy(ai_addr->sin_addr.s4_addr, query_resp.ip_address[i].ipv4_address, 4);
// 			ai->ai_addrlen = sizeof(*ai_addr);
// 			ai->ai_addr = (struct sockaddr *)ai_addr;
// 		} else {
// 			ai_addr6->sin6_family = ai->ai_family;
// 			ai_addr6->sin6_port = htons(port);
// 			memcpy(ai_addr6->sin6_addr.s6_addr, query_resp.ip_address[i].ipv6_address, 16);
// 			ai->ai_addrlen = sizeof(*ai_addr6);
// 			ai->ai_addr = (struct sockaddr *)ai_addr6;
// 		}		
// 	}
...
// static void rs9116w_off_freeaddrinfo(struct zsock_addrinfo *res)
// {
// 	__ASSERT_NO_MSG(res);
...
// 	.getaddrinfo = rs9116w_off_getaddrinfo,
// 	.freeaddrinfo = rs9116w_off_freeaddrinfo,
// };
